...
<template>
  <header
    class="relative z-10 h-[4.5rem] shrink-0 border-b border-transparent bg-white/70 pt-[env(safe-area-inset-top)] backdrop-blur-[10px]"
  >
    <div
    ...
    >
      ...
      <button
        type="button"
        class="ml-auto flex size-12 shrink-0 cursor-pointer flex-col items-center justify-center gap-[0.35rem] rounded-full border border-gray-200 bg-white/90 p-0 shadow-sm backdrop-blur-sm transition-[background,box-shadow,transform] duration-200 hover:scale-[1.02] hover:bg-white hover:shadow-md lg:hidden"
      ...
      >
        <span
          class="block h-0.5 w-5 bg-gray-900 transition-transform duration-300"
          :class="{ 'translate-y-[0.45rem] rotate-45': isOpen }"
        />
        <span
          class="block h-0.5 w-5 bg-gray-900 transition-opacity duration-300"
      ...
      <nav class="ml-auto hidden items-center justify-end gap-6 lg:flex xl:gap-11">
        <RouterLink
          ...
          :key="link.label"
          :to="link.to"
          class="text-[0.875rem] text-gray-700 no-underline transition-colors hover:text-gray-900 router-link-active:text-gray-900 xl:text-[0.95rem]"
        ...
        <RouterLink
          ...
          class="ml-2 inline-flex min-h-12 items-center gap-2 rounded-full bg-gray-900 px-4 py-2.5 text-[0.875rem] font-medium text-white no-underline transition-colors hover:bg-gray-800 xl:px-5 xl:text-[0.95rem]"
